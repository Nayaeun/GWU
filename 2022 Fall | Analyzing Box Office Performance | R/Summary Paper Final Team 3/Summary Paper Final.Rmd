...
We combined three different datasets, IMDB, Rotten Tomatoes, and Box Office Mojo to create our master dataset to answer the SMART question: What factors affect box office performance sales in the US? The IMDB dataset is publicly available online, the Rotten Tomato dataset was pulled from Kaggle, and the Box Office dataset was obtained from Box Office Mojo. 

We joined these datasets through an inner join on movie title, where the year of release ranged from 1912 to 2022. Due to the nature of combining three different datasets, there were multiple columns that were irrelevant to answering our SMART question. After removing those columns, we were left with 8 features to predict our target variable, domestic lifetime gross. After removing unnecessary variables and null values, we converted `contentR` into factor and `Runtime` into numeric variables for our future analysis. We converted Content rating to an ordinal factor with levels G, PG, PG-13, R, NC-17, NR. We also converted several variables from characters to datetime or numerical variables, but these ended up not being used in the analysis.
...
From the scatterplot and correlation test for `imdbR`, it's apparent that there is a weak correlation between `imdbR` and `LifetimeGross` with low correlation coefficient value, `r cor.test(nooutlierR$imdbR, nooutlierR$LifetimeGross)$estimate`. ($p-value$ approximately `r cor.test(nooutlierR$imdbR, nooutlierR$LifetimeGross)$p.value`)
...
Our hypothesis for `imdbR`:

$H_0:$ There is no difference between groups.

$H_a:$ There is some group difference.

The $mean$ for middle and high rating groups are `r ttestimdbR$estimate` respectively. The $p-value$ for these two groups is `r ttestimdbR$p.value`, thus rejected the null that these two groups are the same.

#### 4.1.2 Rotten Tomatoes Rating

##### 4.1.2.1 Scatterplot and Correlation Test

As for `RottentmtR`, it exhibits similar behavior as `imdbR` data as well. It is even more clear that there is weak correlation between `rottentmtR` and `LifetimeGross` with a low correlation coefficient value, `r cor.test(nooutlierR$rottentmtR, nooutlierR$LifetimeGross)$estimate`. ($p-value$ approximately `r cor.test(nooutlierR$rottentmtR, nooutlierR$LifetimeGross)$p.value`)
...
ttestrtR_midtohigh
```
Our hypothesis for `rottentmtR` of $low$ and $middle$ groups:

$H_0:$ There is no difference between groups.

$H_a:$ There is some group difference.

The $mean$ for low and middle rating groups are `r ttestrtR_lowtomid$estimate` respectively. The $p-value$ for these two groups is `r ttestrtR_lowtomid$p.value`, thus rejected the null that these two groups are the same.

Our hypothesis for `rottentmtR` of $middle$ and $high$ groups:

$H_0:$ There is no difference between groups.

$H_a:$ There is some group difference.

The $mean$ for middle and high rating groups are `r ttestrtR_midtohigh$estimate` respectively. The $p-value$ for these two groups is `r ttestrtR_midtohigh$p.value`, thus rejected the null that these two groups are the same.
...
From the scatterplot and correlation test for `imdbC`, it's apparent that there is a correlation between `imdbC` and `LifetimeGross` with  meaningful correlation coefficient value, `r cor.test(nooutlierC$imdbC, nooutlierC$LifetimeGross)$estimate`. With 95% confidence, the correlation is between `r cor.test(nooutlierC$imdbC, nooutlierC$LifetimeGross)$conf.int` ($t\:value$ = `r cor.test(nooutlierC$imdbC, nooutlierC$LifetimeGross)$statistic`, $p-value$ approximately `r cor.test(nooutlierC$imdbC, nooutlierC$LifetimeGross)$p.value`).
...
#### 4.2.2 Rotten Tomatoes Count

##### 4.2.2.1 Scatterplot and Correlation Test
From the scatterplot and correlation test for `rottentmtC`, it's apparent that there is a significant correlation between `rottentmtC` and `LifetimeGross` with  meaningful correlation coefficient value, `r cor.test(nooutlierC$rottentmtC, nooutlierC$LifetimeGross)$estimate`. With 95% confidence, the correlation is between `r cor.test(nooutlierC$rottentmtC, nooutlierC$LifetimeGross)$conf.int` ($t\:value$ = `r cor.test(nooutlierC$rottentmtC, nooutlierC$LifetimeGross)$statistic`, $p-value$ approximately `r cor.test(nooutlierC$rottentmtC, nooutlierC$LifetimeGross)$p.value`).
...
We fit several models with the goal of finding which combination of predictors worked best for predicting __domestic lifetime gross__. Out initial strategy was to use a correlation plot and forward selection to find the best combination of numerical predictors. Then, due to our finding in the EDA process, we added __content rating__ to the best model to see if that improved model performance. We found that the best model included number of __IMDb ratings__, number of __Rotten Tomatos ratings__, and __Content Rating__ as predictors for domestic lifetime gross. It is worth noting that during the EDA process, we did discover that several model conditions were not being met in particular the normality assumption. However, because our somple size is so large, we still proceeded to fit these linear models. 

### 5.2 Model Results Summary and Interpretations

#### 5.2.1 Model 1: Lifetime Gross = bo + b1xIMDbC
...
$H_0:$ b1 = 0 (IMDb count has no significant effect on Lifetime gross)

$H_a:$ b1 != 0 (There is some effect)

The $t-statistic$ for the coefficient on `imdbC` is `r summary(lm1)$coefficients[2,3]`, which gives a $p-value$ of approximately `r summary(lm1)$coefficients[2,4]`. Therefore we can reject the null hypothesis and conclude that there is strong evidence that `imdbC` has a positive relationship with `LifetimeGross`.

#### 5.2.2 Model 2: Lifetime Gross = bo + b1xIMDbCount + b2xRTCount
...
$H_0:$ b2 = 0 (Rotten Tomato count has no significant effect on Lifetime gross)

$H_a:$ b2 != 0 (There is some effect)

The $t-statistic$ for the coefficient on `rtCount` is `r summary(lm2)$coefficients[2,3]`. This gives a $p-value$ of approximately `r summary(lm2)$coefficients[2,4]`. Therefore, there is strong evidence that the number of rotten tomatoes reviews has a statistically significant positive relationship with `LifetimeGross`.

#### 5.2.3 Model 3: Lifetime Gross = bo + b1xIMDbCount + b2xRTCount + b3xruntime
...
$H_0:$ b3 = 0 (Runtime has no significant effect on Lifetime gross)

$H_a:$ b3 != 0 (There is some effect)

The $t-statistic$ for `Runtime` is `r summary(lm3)$coefficients[2,3]`, which gives a $p-value$ of 0.067. There is no evidence that there is a statistically significant relationship between `Runtime` and `LifetimeGross`. For this reason, we rejected this predictor from our model.

#### 5.2.4 ANOVA: Models 1, 2, & 3
```{r anova, results='markup'}
anova1 <- anova(lm3, lm2, lm1)
anova1
```

The $F-statistic$ comparing models 2 and 3 is `r anova1[["F"]][2]`, which gives a $p-value$ of 0.0667. Therefore there is no evidence that model 3 is performing significantly better than model 2, therefore we rejected model 3. The $F-statistic$ comparing models 1 and 2 is `r anova1[["F"]][3]`, which gives a $p-value$ of approximately `r anova1[["Pr(>F)"]][3]`. Therefore, there is strong evidence that model 2 performs better than model 1, therefore we accepted __model 2__ as our best model.

#### 5.2.5 Model 4: Lifetime Gross = bo + b1xIMDbCount + b2xRTCount + *b3*XcontentRating

$H_0:$ all b3i = 0, where i represents the levels of content rating (content rating has no significant effect on Lifetime gross)

$H_a:$ some b3i != 0 (Some level of content rating has an effect)
...
The coefficients for all ratings have $p-values$ of approximately `r summary(lm4)$coefficients[2,4]`. For this reason, we can reject our null hypothesis and say that at least one coefficient for `contentR` is not zero. There is strong evidence that `contentR` has a statistically significant relationship with `LifetimeGross`. 

#### 5.2.6 ANOVA: Models 2 & 4
```{r anova cont rating, results='markup'}
anova2 <- anova(lm4, lm2)
anova2
```

The $F-statistic$ for comparing models 2 and 4 is `r anova2[["F"]][2]`, which gives a $p-value$ of approximately `r anova2[["Pr(>F)"]][2]`. Therefore there is strong evidence that model 4, which includes content rating, is performing statistically significantly better than model 2. We selected __model 4__ as our final model.
...
Box Office Mojo. Retrieved October 2, 2022, from https://www.boxofficemojo.com
IMDB. Retrieved October 2, 2022, from https://www.imdb.com/interfaces/
Rotten Tomatoes. Retrieved October 2, 2022, from https://www.kaggle.com/datasets/stefanoleone992/rotten-tomatoes-movies-and-critic-reviews-dataset
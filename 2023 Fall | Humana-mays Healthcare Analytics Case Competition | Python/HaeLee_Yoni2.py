...
# Train a Random Forest model to get feature importances (as you did in your code)
...
# Select the top k most important features
k = 10  # You can choose the number of features you want to keep
selected_features_rf = X.columns[np.argsort(feature_importances_rf)[-k:]]

# Apply Feature Scaling (Standardization) to selected features
scaler = StandardScaler()
X_scaled = scaler.fit_transform(X[selected_features_rf])

# Dimensionality Reduction (PCA) on scaled features
pca = PCA(n_components=2)  # Choose the number of components you want
X_pca = pca.fit_transform(X_scaled)

# Split data into training and testing sets for both Random Forest and Neural Network
X_train_rf, X_test_rf, y_train_rf, y_test_rf = train_test_split(X_pca, y, test_size=0.2, random_state=42)
X_train_nn, X_test_nn, y_train_nn, y_test_nn = train_test_split(X_scaled, y, test_size=0.2, random_state=42)

#========================Haelee RF============================
# Train a classifier (Random Forest) and evaluate its performance (e.g., accuracy)
...
# Train a classifier (Random Forest) and evaluate its performance (e.g., accuracy)
clf_rf = RandomForestClassifier(random_state=42, **best_params)
clf_rf.fit(X_train_rf, y_train_rf)
y_pred_rf = clf_rf.predict(X_test_rf)
accuracy_rf = accuracy_score(y_test_rf, y_pred_rf)

...
library(ezids)
library(stringr)
knitr::opts_chunk$set(echo=TRUE, warning=FALSE, fig.align = 'center', error = F, message = F)
options(scipen=999)
library(purrr)
```
...
Feature Selection: nbest= 2, nvmax = 10
...
Four models were fit based on the adjusted $R^2$ value. 

All models have very low p-value, much lower than .05. 
Best model: model 2; lowest p-value along with greater simplicity over model 4
...
summary(comedyglm)
```
The best model has AIC 9623.153 with all variables except `numVotes`.
...
Criteria: AIC and BIC; some genre variables not considered
```{r,include=FALSE}
library(glmulti)
```
```{r}
...
glmulti.aic <- glmulti(rotten~., data=movie_cl, #model formula and data
                       ...
                       method='d') 
```
Number of combinations: from 524288 to 32768 (93% reduction)

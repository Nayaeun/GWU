...
Based on the correlation matrix of the numerical variables, the first feature we chose to run a regression on was imdbCount. After running this model, we got an R^2 value of 0.3679, which is not bad, but could certainly be improved upon. Both the intercept term and the coefficient for IMDb count are highly significant. As number of IMDb ratings increase by 1, the domestic lifetime gross increases by about 219USD.
...
In order to see if we  could improve the R^2 value, we added the explanatory variable with the next highest correlation with domestic lifetime groos, which is Rotten Tomatos review count. This model increases he R^2 value to 0.4296, which for non-scientific data is fairly decent. Both coefficients are highly significant in the model. When running a one way ANOVA for this model and our original model, we get an F-stat of 838.3 (p value approx 0), which means model 2 is performing much better than the reduced model. 
...
Following the same logic as with model two, we added runtime as a third explanatory variable. This coefficient is not significant in the model and has no impact on the R^2 value. Running a one way ANOVA confirms that model 3 does not perform better than model 2 (p value=0.0667), so Model 2 is our best model using only numerical variables.
...
## Content Rating
The model below includes content rating as a factor. This improves the R^2 to 0.4697, and every coefficient for each level is statistically significant.
...
anova(lm4, lm2)
```
As we can see from the above ANOVA, the final model which includes content rating is statistically significantly better than model 2 (F = 118.41)

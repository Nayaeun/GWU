...
For the midterm, we combined datasets from IMDB, Rotten Tomatoes, and Box Office Mojo, to answer our SMART question: What factors affect box office performance sales in the US? We joined these three datasets through an inner join on movie title, where the year of release ranged from 1912 to 2022. We analyzed lifetime gross as our response variable, and for the final project, we decided to continue our analysis of our combined dataset with different SMART questions.
...
We created one-hot indicator variables for the top genres using the 'ifelse' and 'str_detect' functions.
```{r}
movie_red <-
  movie_red %>%
...
We created one-hot indicator variables for the Tomatometer Status using the 'ifelse' and 'str_detect' functions.
```{r}
movie_red <-
  movie_red %>%
...
The IMDB Rating was very close to a normal distribution, therefore, no skew affected the model or analysis.
...
For the feature selection, both qualitative and quantitative variables were included. With the exhaustive search, the criteria of `nvmax=10` and `nbest=2` were used, which means that the plot will show two subsets of a maximum of 10 variable models. This will ultimately assist in creating the best model. Although the qualitative variables are broken down into their respective levels, it still gives a good idea as to which variables should be run in the models. The adjusted $R^2$  plot shows different sets of features that would produce a good model. These variables include: content rating, runtime, tomatometer rating, tomatometer count, tomato meter status, IMDB votes, and the genres- drama, comedy, horror, and biography. 
...
For the IMDB rating model, the model will be a linear regression since the response variable is continuous. With this model, the $R^2$ value quantifies the variance of our dependent variable, explained by our independent variables. We used adjusted $R^2$ because it takes into account the number of variables and penalizes a large number of variables, whereas $R^2$ favors models with an increasing number of variables. Due to having many independent variables, especially with each genre being considered as a separate variable, we decided to use the adjusted $R^2$ metric.
...
summary(fit2)
#adding qual vars- content rating and tomatometer_status
...
After counting genre, we decided to select top 3 genres, drama, comedy, and romance to predict the most affected feature to the genre. 

## Skewness of genres
We are predicting the variables with an indicator variable, which is whether the movie genre is 'drama' or not. In addition, since from the count table on the top, we already selected the top 3 genres that have about 50% proportion from total genre counts.

Firstly, let's look at drama genre.
...
Variables considered for feature selection:
```{r}
# Make new dataset for feature selection for drama genre
movieclean <- movie_red %>%
  select(content_rating, runtime, tomatometer_rating, lifetime_gross, startYear, imdbRating, numVotes)
 movieclean <-
   movieclean %>%
...
```{r, results='hide'}
movieclean$y = movie_red[,13]
movieclean$y <- as.factor(movieclean$y)
str(movieclean)
```

The best model after exhaustive search:
```{r}
library("leaps")
library("bestglm")
res.bestglm <-
...
summary(dramaglm)
```
According to feature selection through AIC, the best model has AIC 9840 with all variables, `content_rating`, `runtime`, `tomatometer_rating`, `lifetime_gross`, `imbdRating`, and `numVotes` except `year`.
...
But, the p-value of `r dramaHoslem$p.value` is relatively high. This indicates the model is not really a good fit, despite all the coefficients are significant. 

## Comedy Genre

```{r, results='hide'}
# comedy indicator variable into y variables
movieclean$y = movie_red[,14]
movieclean$y <- as.factor(movieclean$y)
str(movieclean)
```
...
summary(comedyglm)
```
According to the future selection with AIC, the best model has AIC 9643 with all variables except `numVotes`.
...
Also, the p-value of `r comedyHoslem$p.value` is relatively high. This indicates the model is not really a good fit, despite all the coefficients are significant.

## Romance Genre
```{r, results='hide'}
# romance indicator variable into y variables
movieclean$y = movie_red[,15]
movieclean$y <- as.factor(movieclean$y)
str(movieclean)
```
...
summary(romanceglm)
```
According to the result of feature selection with AIC, the best model has AIC 7282 with all variables, `content_rating`, `runtime`, `tomatometer_rating`, `lifetime_gross`, `year`, `imbdRating`, and `numVotes`.
...
Also, the p-value of `r romanceHoslem$p.value` is relatively high. This indicates the model is not really a good fit, despite all the coefficients are significant.

Displaying feature selection with AIC, we conclude that `content_rating`, `runtime`, `tomatometer_rating`, `lifetime_gross`, `imdbRating` are decisive features to all genres which are drama, comedy, and romance. Specially, `numVotes` variable affect only drama and romance genre and `year` variables affect only comedy and romance genre.
...
```
- Note that we had run this 10 hour long exhaustive search and results are the same as that run below. 
- We have reduced the total number of variables to 15, eliminating the genres with least count (these eliminated variables played almost no role in the results we got from the full search).
...
- We could not figure out how to plot AUC with repeated CV, so we are doing it with a train-test split
